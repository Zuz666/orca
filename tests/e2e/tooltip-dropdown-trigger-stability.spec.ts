...
const SOURCE_CONTROL_TRIGGER_CASES = [
  ['More commit history actions', 'More commit history actions'],
...
async function exerciseCommitPathControls(page: Page): Promise<void> {
  await page.getByRole('button', { name: 'More commit history actions' }).click()
  const viewAsTree = page.getByRole('menuitem', { name: 'View as tree' })
  await ((await viewAsTree.count()) > 0 ? viewAsTree.click() : page.keyboard.press('Escape'))

  const commitRows = page.locator('button[data-testid="git-history-row"]')
  if ((await commitRows.count()) === 0) {
    await page.getByRole('button', { name: /^Commits(?:\s+\d+\+?)?$/ }).click()
  }

  const commit = page.locator('button[data-testid="git-history-row"]').last()
  await expect(commit).toBeVisible({ timeout: 10_000 })
  await commit.click()

  const file = page.locator('button[data-testid="git-history-commit-file"]').first()
  await expect(file).toBeVisible({ timeout: 10_000 })
  const filePath = await file.getAttribute('data-file-path')
  if (!filePath) {
    throw new Error('Commit file row is missing its relative path')
  }
  await expectTooltipOwnedByButton(page, file, filePath)

  const directory = page.locator('button[data-testid="git-history-commit-directory"]').first()
  await expect(directory).toBeVisible()
  const directoryPath = await directory.getAttribute('data-tree-path')
  if (!directoryPath) {
    throw new Error('Commit directory row is missing its tree path')
  }
  await expectTooltipOwnedByButton(page, directory, directoryPath)
  ...
  await commit.click()
  const reopenedDirectory = page
    .locator('button[data-testid="git-history-commit-directory"]')
    .first()
  await expect(reopenedDirectory).toHaveAttribute('aria-expanded', 'false')

  const refresh = page.getByRole('button', { name: 'Refresh commits' })
  await refresh.focus()
  await refresh.click()
  ...
  test('Source Control compound triggers survive focus, menus, and remounts', async ({
    ...
    try {
      for (let cycle = 0; cycle < 2; cycle += 1) {
        ...
        if (cycle === 0) {
          await exerciseCommitPathControls(orcaPage)
        }
